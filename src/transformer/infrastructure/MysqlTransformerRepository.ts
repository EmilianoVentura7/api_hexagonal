...
export class MysqlTransformerRepository implements TransformerRepository {
  ...
  async getById(id_transformer: number): Promise<Transformer | null> {
    ...
    const sql = "SELECT * FROM transformer WHERE id=?"; //cambiar query
  ...
  async getByUser(id_user: number): Promise<Transformer | null> {
    ...
    try {
      const [result]: any = await query(sql, params);
      return new Transformer(result[0].id, result[0].status, result[0].id_user);
    } catch (error) {
      return null;
    }
  }
  ...
  async createTransformer(
    ...
    const sql = "INSERT INTO transformer (id_user, status) VALUES (?, ?)";
    const params: any[] = [id_user, status];
    try {
      const [result]: any = await query(sql, params);
